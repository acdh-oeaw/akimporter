	...
	private void startImporting() {
		...
		if (typeOfDataset.equals("1")) { // We have one big XML file
			...
			if (isMergeOk.equals("Y")) {
				...
				isMergingSuccessful = xmlm.mergeElements(pathToMultipleXmlFolder, pathToMergedFile, "collection", "record", 1, null, null);
			...
			if (hasValidationPassed) {
				...
				if (isIndexingOk.equals("Y")) {
					...
					if (this.optimize) {
						...
						AkImporterHelper.solrOptimize(solrServer);
					}

					if (isIndexingSuccessful && isRelateSuccessful) {
						endTime = System.currentTimeMillis();
						AkImporterHelper.print(this.print, "\nDone importing. Everything successful. Execution time: " + AkImporterHelper.getExecutionTime(startTime, endTime) + "\n");
					} else {
